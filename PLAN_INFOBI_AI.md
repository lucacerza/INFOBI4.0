...
- [x] **ETL incrementale schedulato (APScheduler, watermark)** — completo:
  - [x] **Step 1 — carico incrementale (append) con watermark**: campi `WarehouseDataset` (sync_mode/watermark_column/last_watermark/key_columns); `warehouse.materialize_incremental` (primo carico full → poi `WHERE wm > last_watermark` + append, avanza il watermark); `refresh`/`from-report` rispettano `sync_mode` (validazione: incrementale richiede watermark_column). Test `test_warehouse_incremental.py`.
  - [x] **Step 2 — merge/upsert su `key_columns`**: `_merge_df` (DELETE chiavi in arrivo + INSERT, idempotente; chiavi singole o composite via row-value IN); merge se `key_columns` valorizzato, altrimenti append; modo `noop` se nessuna riga nuova. Test: update non duplica (service + e2e).
  - [x] **Step 3 — scheduling + UI**: logica condivisa `services/warehouse_sync.py` (`sync_dataset` + `refresh_all`); job APScheduler opt-in (`WAREHOUSE_SYNC_ENABLED`/`WAREHOUSE_SYNC_INTERVAL_HOURS`); UI nel pannello Warehouse (ReportViewer): select modalità full/incrementale + watermark_column + chiavi merge + watermark corrente. Refactor: l'API `_materialize` ora delega a `sync_dataset`.
...
- [x] **8.5 — NL → Dashboard**: `services/nl_dashboard.py` — tool `build_dashboard` (lista widget chart/grid con chart_type/group_by/split_by/metrics), riusa grounding + `validate_and_build` per ogni widget (stesso guardrail); produce config nella forma del viewer (`chartType/groupBy/splitBy/metrics`). Endpoint `POST /api/ai/reports/{id}/dashboard` (admin) crea Dashboard + widget con auto-layout e ritorna l'id (loggato). UI: blocco "Genera dashboard con l'AI" nel ReportViewer → naviga alla dashboard creata. Test `test_nl_dashboard.py` (3: creazione+config widget, guardrail, descrizione vuota).
...
- [x] **8.6 — Anomaly detection**: `services/anomaly.py` — statistica pura (z-score / IQR), deterministica, niente LLM; riusa grounding/guardrail + motore pivot per l'aggregazione, individua e ordina gli outlier (media/dev std, direzione high/low). Endpoint `POST /api/ai/reports/{id}/anomalies` (group_by + metric + method/threshold). UI: bottone "Anomalie" nel ReportPivotPage (riusa il pannello). Test `test_anomaly.py` (7: z-score/IQR, pochi punti, serie costante, endpoint, group_by mancante, guardrail metrica).
- [x] **8.7 — Forecasting (time-series)**: `services/forecast.py` — regressione lineare (minimi quadrati) con banda ~95% dai residui; statistica pura, no LLM; riusa grounding/guardrail + motore pivot; continuazione numerica delle etichette tempo (es. anni) + trend up/down/flat. Endpoint `POST /api/ai/reports/{id}/forecast` (time_field + metric + periods). UI: bottone "Previsione" nel ReportPivotPage. Test `test_forecast.py` (5: fit perfetto, <2 punti, endpoint, storico insufficiente, guardrail).
- [x] **8.8 — Chatbot sul catalogo (RAG)**: `services/catalog_rag.py` — corpus dai metadati (report + semantic layer), retrieval per termini (no vector DB, no dipendenze), LLM risponde SOLO sul contesto recuperato citando i report. Endpoint `POST /api/ai/catalog/chat` (loggato, kind=catalog_chat). Test `test_catalog_rag.py` (4: ranking, query vuota di soli stopword, e2e con fonti, domanda vuota).
